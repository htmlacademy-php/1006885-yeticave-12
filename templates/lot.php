...
<head>
  <meta charset="UTF-8">
  <title><?= filter_xss($lot['lot_name']) ?></title>
    ...
    <nav class="nav">
      <ul class="nav__list container">
        <?php foreach ($lots_categories as $category) : ?>
          <li class="nav__item">
              <a href="pages/all-lots.html"><?= filter_xss($category['category_name']); ?></a>
          </li>
        <?php endforeach; ?>
      </ul>
    </nav>
    <section class="lot-item container">
      <h2><?= filter_xss($lot['lot_name']) ?></h2>
      <div class="lot-item__content">
        <div class="lot-item__left">
          <div class="lot-item__image">
            <img src="<?= filter_xss($lot['img_url']) ?>" width="730" height="548" alt="">
          </div>
          <p class="lot-item__category">Категория: <span><?= filter_xss($lot['category_name']) ?></span></p>
          <p class="lot-item__description"><?= filter_xss($lot['lot_desc']) ?></p>
        </div>
        <div class="lot-item__right">
          <div class="lot-item__state">
            <div class="lot-item__timer timer <?php if (get_time_interval($lot['date_exp'])['hh'] < 1) : ?>timer--finishing<?php endif; ?>">
            ...
            <div class="lot-item__cost-state">
              <div class="lot-item__rate">
                ...
                <span class="lot-item__cost"><?= filter_xss($lot['lot_price']) ?></span>
              </div>
              <div class="lot-item__min-cost">
                Мин. ставка <span><?= filter_xss($lot['lot_price'] + $lot['rate_step']) ?></span>
              </div>
            </div>
